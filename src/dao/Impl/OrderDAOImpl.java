...
public class OrderDAOImpl extends DAO<OrderBean> {
     ...
     * 获取所有可以接的任务
     * @return
     * 这里修改过了，若OrderTable 数据量大了，做2表的笛卡尔积 效率会非常低，所以这里的sql可以优化
     */
    public List<OrderBean> getOrderAll() {
//        String sql = "select * from OrderTable where state = 0";
        String sql = "select ot.order_id,ot.release_student_number,ot.time,ot.startLocation,ot.endLocation,ot.content,ot.lable,ot.tip,ot.acceptance_student_number,ot.score,ot.state,ut.student_name,ut.sex,ut.cell_phone," +
                "ut.school,complaint_content,complaint_time,complaint_student_number"
                +" from OrderTable ot,usertable ut"
                +"   where state = 0"
                +" AND ot.release_student_number = ut.student_number" +
                " ORDER BY time desc";
...
//        String sql = "select * from OrderTable where " +OrderServlet.RELEASE_STUDENT_NUMBER + " = ?";
        String sql = "select ot.order_id,ot.release_student_number,ot.time,ot.startLocation,ot.endLocation,ot.content,ot.lable,ot.tip,ot.acceptance_student_number,ot.score,ot.state,ut.student_name,ut.sex,ut.cell_phone," +
                "ut.school,complaint_content,complaint_time,complaint_student_number"
                +" from OrderTable ot,usertable ut"
                +"  where "+OrderServlet.RELEASE_STUDENT_NUMBER +" = ?"
                +" AND ot.release_student_number = ut.student_number" +
                " ORDER BY time desc";
        return getForList(sql , orderBean.getRelease_student_number());
    }
...
//        String sql = "select * from OrderTable where " + OrderServlet.ACCEPTANCE_STUDENT_NUMBER + " = ?";
        String sql = "select ot.order_id,ot.release_student_number,ot.time,ot.startLocation,ot.endLocation,ot.content,ot.lable,ot.tip,ot.acceptance_student_number,ot.score,ot.state,ut.student_name,ut.sex,ut.cell_phone," +
                "ut.school,complaint_content,complaint_time,complaint_student_number"
                +" from OrderTable ot,usertable ut"
                +"  where "+OrderServlet.ACCEPTANCE_STUDENT_NUMBER +" = ?"
                +" AND ot.release_student_number = ut.student_number" +
                " ORDER BY time desc";
